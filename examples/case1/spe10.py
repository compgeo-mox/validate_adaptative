...
class Spe10(object):
    ...
    def save_perm(self):
        ...
        for _, d in self.mdg.subdomains(return_data=True):
            ...
            pp.set_solution_values(names[4], self.perm[:, 0] * pp.DARCY, d, 0)
            pp.set_solution_values(names[5], self.perm[:, 1] * pp.DARCY, d, 0)
            pp.set_solution_values(names[6], self.perm[:, 2] * pp.DARCY, d, 0)
