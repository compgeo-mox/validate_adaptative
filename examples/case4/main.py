...
import sys; sys.path.insert(0, "../case1/")
...
def add_wells(mdg, domain, well_coords, well_num_cells, tol):
    ...
    wells_set = pp.WellNetwork3d(wells, domain, parameters={"mesh_size": mesh_size})
...
def main(region):
    ...
    spe10.read_perm("../case1/spe10_perm/")

    hx, hy, hz = spe10.physdims / spe10.shape

    # add the wells
    well_coords = np.asarray([
        np.array([[1.5*hx, 1.5*hx],
                  [1.5*hy, 1.5*hy],
                  [spe10.physdims[2], spe10.physdims[2] - 2.5*hz]]),

        np.array([[spe10.physdims[0] - 1.5*hx, spe10.physdims[0] - 1.5*hx],
...
if __name__ == "__main__":
    ...
    #for sd, data in spe10.mdg.subdomains(return_data=True):
    #    param = {"ambient_dimension": spe10.mdg.dim_max(),
    #             "source": np.zeros(sd.num_cells)}

    #    b_faces = sd.tags['domain_boundary_faces'].nonzero()[0]
    #    bc_type = ["neu"]*b_faces.size

    #    bc_values = np.zeros(sd.num_faces)
    #    if sd.dim == 3:
    #        perm = spe10.perm[:, 0] * pp.DARCY
    #    elif sd.dim == 1 and sd.well_num > -1:
    #        top_face = np.argmax(sd.face_centers[2, :])
    #        if sd.well_num == 0:
    #            bc_values[top_face] = - 10 ##### set a value
    #        else:
    #            bc_values[top_face]  = 10 ##### set a value

    #        param["well_radius"] = 10 * pp.CENTIMETER
    #        perm = np.pi * np.square(param["well_radius"]) #### * self.electrode_sigma #####

    #    param["second_order_tensor"] = pp.SecondOrderTensor(perm*np.ones(sd.num_cells))
    #    param["bc"] = pp.BoundaryCondition(sd, b_faces, bc_type)
    #    param["bc_values"] = bc_values
