...
class Data:
    def __init__(self, spe10, epsilon=None, u_bar=None, region=None, tol=1e-6):
        ...
        rho = 1000.0  # fluid's density [kg/m3]
        ...
        pphi_1 = lambda a: lambda_1 * a + 2 / 3 * beta_1 * np.power(
            np.abs(a), 1.5
        )  # primitive of phi_1
        ...
        pphi_2 = lambda a: lambda_2 * a + 2 / 3 * beta_2 * np.power(
            np.abs(a), 1.5
        )  # primitive of phi_2
    ...
    def vector_source(self, sd, d, flow_solver):
        ...
        vect = np.vstack((coeff, np.zeros(sd.num_cells), np.zeros(sd.num_cells))).ravel(
            order="F"
        )
        return vect
