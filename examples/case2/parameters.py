...
class Parameters:
    ...
    def __init__(self):
        ...
        subcase = "network" # "couple" or "network"
        if subcase == "couple":
            ...
            self.lenses = [lens_v1, lens_v2, lens_v3, lens_h1, lens_h2, lens_h3, lens_h4]

        # to determine critical Forchheimer number
        self.E = 0.1 # maximum error to Forchheimer accepted [-]

        # call internal functions
        file_bg = "porosity"       # file containing background porosity or permeability
    ...
    def _read_background(self, file_bg):
        assert self.data_kind == "poro" or self.data_kind == "perm", \
    ...
    def _compute_and_print(self):
        ...
        if self.data_kind == "poro":
            for l in range(num_lenses):
                print("phi_" + str(l+1) + " =", round(self.lenses[l]["val"]*100, 5), "[%]")
        else:
            for l in range(num_lenses):
