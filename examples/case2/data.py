...
class Data:
    def __init__(self, parameters, problem, folder, tol=1e-6):
        ...
        kappa = problem.kappa  # intrinsic permeability #
    ...
    def bc(self, sd, data, tol, flow_solver):
        ...
        in_flow = b_face_centers[1] > self.problem.full_physdims[1] - tol

        # define outflow type boundary conditions
        out_flow = b_face_centers[1] < 0 + tol

        # get necessary parameters
        u_bar = self.problem.u_bar
        rho = self.parameters.rho
        influx = self.parameters.influx / u_bar  # divide by u_bar for normalization
        atm_pressure = self.parameters.atm_pressure
        g = self.parameters.g
        h = self.parameters.length_y
        ...
        bc_val[b_faces[in_flow]] = -influx * sd.face_areas[b_faces][in_flow]
        bc_val[b_faces[out_flow]] = atm_pressure + rho * g * h  # hydrostatic pressure at bottom
